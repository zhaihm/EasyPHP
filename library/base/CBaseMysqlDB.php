...
class CBaseMysqlDb
{
    ...
    public function countByArray($tablename,$params=array())
    {
        ...
        $sql = "select count(*) as count from {$this->database}.$tablename t $conditionstr";
    ...
    public function endMultiTransaction()
    {
        ...
        $ret = $this->executetransaction($this->sqls);
        $this->sqls = array();
        return $ret;
    }
    ...
    public function executetransaction($sqls,$printlog=true,$errorlog=true)
    {
        $affected_rows = 0;
        $tBegin = Log::getLogTime();
        ...
        if($ret === false)
        {
            ...
            if($errorlog){
                Log::WriteLog('execute sql['.var_export($sqls,true).'] failed, will rollback, errorno['.mysql_errno().'], error message['.mysql_error().'], rollback sqls['.var_export($sqls,true).']','error', $tEnd-$tBegin, 'dbinfo');
            }
            return false;
        }
        ...
        if($printlog){
            Log::WriteLog("database[{$this->database}]   sqls[".var_export($sqls,true)."]    rows_affected[" . var_export($affected_rows,true) . ']', 'info', $tEnd-$tBegin, 'dbinfo');
        }
        return true;
