...
class CHttpRequestInfo
{
    ...
    private static function Get($key,$default='')//support array in_params
    {
    ...
    private static function GetArray(&$inarr)
    {
    ...
    private static function PostArray(&$inarr)
    {
        foreach($inarr as $key => $value)
        {
    ...
    public static function Param($key,$required=true,$default='') {
        $data = self::Post($key);
        if(strlen($data) === 0){
        ...
        if (strlen($ret) === 0) {
            if ($required) {
                throw new Exception($key.' cannot be empty', -1);
            } else {
                $ret = $default;
            }
        }
    ...
    public static function ParamArray($keys, $required=true, $default = array()) {
        $data_list = array();
        for ($i = 0; $i < count($keys); $i++) {
            $default_value = isset($default[$i]) ? $default[$i] : '';
            $value = self::Param($keys[$i], $required, $default_value);
            if (strlen($value) != 0) {
                $data_list[$keys[$i]] = $value;
            }
        }
        return $data_list;
    }

    public static function ParamList($keys, $required=true, $default = array()) {
        $ret = array_values(self::ParamArray($keys, $required, $default));
        while (count($ret) < count($keys)) {
            $ret[] = null;
        }
        return $ret;
    }
